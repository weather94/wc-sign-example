...
function App() {
  ...
  const extensionSwap = useCallback(async () => {
    if (extensionAddress) {
      const address = extensionAddress;
      const url = `${LCD_ENDPOINT}/cosmos/auth/v1beta1/accounts/${address}`;
      axios.get(url).then((response) => {
        ...
        extensionConnector
          ...
          .then((response) => {
            ...
            const signature = response.signature;
            const pub_key = response.pub_key;
            return broadcastSwapTx(signed, { signature, pub_key });
          })
          .then((result) => {
            const code = _.get(result, "code");
            if (code === 0) {
  ...
  const extensionDeposit = useCallback(async () => {
    if (extensionAddress) {
      const address = extensionAddress;
      const url = `${LCD_ENDPOINT}/cosmos/auth/v1beta1/accounts/${address}`;
      axios.get(url).then((response) => {
        ...
        extensionConnector
          ...
          .then((response) => {
            ...
            const signature = response.signature;
            const pub_key = response.pub_key;
            return broadcastDepositTx(signed, { signature, pub_key });
          })
          .then((result) => {
            const code = _.get(result, "code");
            if (code === 0) {
  ...
  const extensionWithdraw = useCallback(async () => {
    if (extensionAddress) {
      const address = extensionAddress;
      const url = `${LCD_ENDPOINT}/cosmos/auth/v1beta1/accounts/${address}`;
      axios.get(url).then((response) => {
        ...
        extensionConnector
          ...
          .then((response) => {
            ...
            const signature = response.signature;
            const pub_key = response.pub_key;
            return broadcastWithdrawTx(signed, { signature, pub_key });
          })
          .then((result) => {
            const code = _.get(result, "code");
            if (code === 0) {
          ...
          <div>
            ...
            {extensionAddress && (
              <>
                ...
                <br />
                <Button type="primary" onClick={extensionSwap}>
                  Swap
                </Button>
                <Button type="primary" onClick={extensionDeposit}>
                  Deposit
                </Button>
                <Button type="primary" onClick={extensionWithdraw}>
                  Withdraw
                </Button>
              </>
            )}
